...
/**
 * The main interface of the chess chess.model. It provides all necessary methods for accessing and
 * manipulating the data such that a game can be played successfully.
 *
 * <p>When something changes in the chess.model, the chess.model notifies its observers by firing a {@link
...
public interface Model extends PropertyChangeListener {
  ...
  /**
   * Sets the established server as an attribute of the corresponding chess.model.
   *
  ...
  /**
   * Sets the player of this chess.model for the chess.network game.
   *
  ...
  /**
   * Gets the player of this chess.model in a chess.network game.
   *
  ...
  /**
   * Set up the client for the chess.network game.
   *
   * @param server the InetAddress of the server
   * @return the client that was set up
   * @throws ConnectException exception that can occur within the chess.network application
   */
  Client setUpClient(InetAddress server) throws IOException;

  /**
   * Set up the server for the chess.network game.
   *
   * @throws IOException exception that can occur within the chess.network application
   */
  void setUpServer() throws IOException;

  /**
   * Set the received state as the current state of the chess.model the client is bound to.
   *
  ...
  /**
   * Add a {@link PropertyChangeListener} to the chess.model that will be notified about the changes made
   * to the chess board.
   *
   * @param pcl the chess.view that implements the listener.
   */
  void addPropertyChangeListener(PropertyChangeListener pcl);

  /**
   * Remove a listener from the chess.model, which will then no longer be notified about any events in the
   * chess.model.
   *
   * @param pcl the chess.view that then no longer receives notifications from the chess.model.
   */
